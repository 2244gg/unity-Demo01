...
public class BundleEditor
{
    ...
    private static string ABCONFIGPATH = "Assets/ABFrame/Editor/Resource/ABConfig.asset";
    ...
    public static void Build()
    {
        ...
        AssetDatabase.SaveAssets();// 会耗时
        AssetDatabase.Refresh();// 耗时长
    ...
    {
        string[] allBundles = AssetDatabase.GetAllAssetBundleNames();// 获取所有好的ab包
        //key为全路径，value为包名
        ...
        for (int i = 0; i < allBundles.Length; i++)
        {
            ...
            for (int j = 0; j < allBundlePath.Length; j++)
            {
                ...
                //if(ValidPath(allBundlePath[j]))//----------------------只打包有效路径
    ...
    static void WriteData(Dictionary<string ,string> resPathDic)
    {
        ...
        foreach (string path in resPathDic.Keys)
        {
            ...
            abBase.Path = path;
            abBase.Crc = Crc32.GetCrc32(path);// 生成CRC校验码
            ...
            for (int i = 0; i < resDependce.Length; i++)
            {
                ...
                if (resPathDic.TryGetValue(tempPath, out abName))
                {
                    ...
                    {
                        abBase.ABDependce.Add(abName);//一个ab包中的东西可能依赖另外个AB包
                    }
                }
            }
        ...
        fs.Close();
      /*  // 写入第二份-----------------------------------------------------------------------------------------暂时处理------
        FileStream fs2 = new FileStream(Application.streamingAssetsPath, FileMode.Create, FileAccess.ReadWrite, FileShare.ReadWrite);
        fs2.Seek(0, SeekOrigin.Begin);
        fs2.SetLength(0);
        BinaryFormatter bf2 = new BinaryFormatter();
        bf2.Serialize(fs2, config);
        fs2.Close();*/
    ...
    static bool ContainAllFileAB(string path)
    {
        ...
        {
            // (path.Replace(m_AllFileAB[i],"")[0] == '/')避免将其他包含该文件夹名字的文件夹算进来
